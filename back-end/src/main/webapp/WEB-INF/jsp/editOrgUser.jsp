...
<html>
<head>
	<title>Edit Organization Info</title>
...
<form:form method="post" action="organization/update.html" commandName="user" enctype="multipart/form-data">
	...
	<tr>
	<td><form:label path="enableCustCredit">Enable Customer Credit</form:label></td>
		<td>
		<c:choose>
			<c:when test="${user.enableCustCredit}"><input type="checkbox" onclick="selectCredit()" id="enableCustCredit" name="enableCustCredit" checked/></c:when>
			<c:otherwise><input type="checkbox"  onclick="selectCredit()" id="enableCustCredit" name="enableCustCredit" /></c:otherwise>
		</c:choose>
		</td>
	
	</tr>
	
	<tr id="colSelectCreditLabel" hidden="true"><td > Select Default Customer Credit</td>
			 <td><select name="defaultCreditType">
				<c:forEach items="${creditTypeList}" var="creditTypeList">
					<c:choose>
						<c:when test="${creditTypeList.id == user.defaultCreditType}">
							<option value="${creditTypeList.id}" selected="selected">${creditTypeList.name}</option>
						</c:when>
						<c:otherwise>
							<option value="${creditTypeList.id}">${creditTypeList.name}</option>
						</c:otherwise>
					</c:choose>
				</c:forEach>
			</select></td>
...
<script>
	$(function() {
		...
		if($('#enableCustCredit').is(':checked')){
			$("#colSelectCreditLabel").show();
	}
	});
	...
	if($('#enableCustCredit').is(':checked')){
		...
		$("#colSelectCreditLabel").hide();
		/* $("#dishType select").val("null");
		$("#dishType option:selected").text("Select Default Credit"); */
	}
		
	}
	function removeImage(parameter){
		if (confirm('Do you really want to remove this image')) {
			window.location.href = 'organization/removeImage?parameter='+parameter;
		}
	}
