...
<form:form method="post" action="organization/addRestaurant.html" commandName="restaurant">
	...
	<form:hidden path="parentRestaurantId" value='<%=request.getSession().getAttribute("parentRestaurantId")%>'/>
	<form:hidden id="currency" path="currency"/>
	<table>	
     <tr>
		<td><form:label path="restaurantName">Restaurant Name*</form:label></td>
		<td><form:input path="restaurantName" required="true" maxlength="50"/></td>
	</tr>
	<tr>
	<td colspan="2"><form:errors path="restaurantName" style="color: red; padding-left: 5px;"/></td>
	</tr>
	<tr>
		<td><form:label path="bussinessName">Business Name*</form:label></td>
		<td><form:input path="bussinessName" required="true"  /></td>
	</tr>
	
	<tr>
		<td><form:label path="bussinessPhoneNo">Business Phone No*</form:label></td>
		<td><form:input path="bussinessPhoneNo" required="true"  /></td>
	</tr>
	
	<tr>
		<td><form:label path="address1">Address*</form:label></td>
		<td><form:input path="address1" required="true"  class="validate[maxSize[100]]"/></td> 
	</tr>
	<tr>
	...
	<tr>
		...
		<td><form:input path="city" required="true"  class="validate[maxSize[100]]"/></td>
	</tr>
	<tr>
		<td><form:label path="state">State*</form:label></td>
		<td><form:input path="state" required="true" maxlength="50"  class="validate[maxSize[100]]"/></td>
	</tr>
	<tr>
		...
		<c:forEach items="${countryList}" var="country">
			<c:choose>
					<c:when test="${country eq restaurant.country}">
	...
	<tr>
		...
		<td><form:input type="number" path="zip" class="validate[maxSize[10]]"/></td>
	</tr>
	...
	<tr>
		...
		<td><form:input type="text" path="serviceTaxText" maxlength="50" class="validate[maxSize[50]]"/></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td><form:input path="mailUsername" type="email" step="any" min="0"/></td>
	</tr>
	<tr>
		<td><form:label path="mailPassword">Mail Password</form:label></td>
		<td><form:input path="mailPassword" type="password" step="any" min="0"/></td>
	</tr>
	<tr>
		<td><form:label path="mailHost">Mail Host</form:label></td>
		<td><form:input path="mailHost" type="text" step="any" min="0"/></td>
	</tr>
	<tr>
		<td><form:label path="mailPort">Mail Port</form:label></td>
		<td><form:input path="mailPort" type="text" step="any" min="0"/></td>
	</tr>
	...
	<c:choose>
		<c:when test="${statusType == restaurant.status }">
	...
	<select name=cur id=cur onChange=list(this.value)>
		...
		var actualCurrCode = "${restaurant.currency}";
		...
		<td>
			<select name="timeZone">
				<c:forEach items="${timeZones}" var="tz">
					<c:choose>
						<c:when test="${tz == restaurant.timeZone}">
	...
	<td><c:choose>
		<c:when test="${restaurant.roundOffAmount}"><input type="checkbox" id="roundOffAmount" name="roundOffAmount" checked /></c:when>
	...
	<td><c:choose>
		<c:when test="${restaurant.deliveryManagerEdit}"><input type="checkbox" id="deliveryManagerEdit" name="deliveryManagerEdit" checked /></c:when>
		...
		<td>
			<select name="openFlag">
				<c:forEach items="${openFlag}" var="oF">
					<c:choose>
						<c:when test="${oF == restaurant.openFlag}">
	...
	<tr>
		...
		<td><textarea id="closedText" name="closedText"  placeholder="Closed Text" style="width:680px;" >${restaurant.closedText}</textarea></td>
	</tr>
	...
	<tr>
		...
		<c:choose>
			<c:when test="${fn:startsWith(restaurant.closeImageLink, 'http://')}">(${restaurant.closeImageLink})</c:when>
			<c:when test="${fn:startsWith(restaurant.closeImageLink, '/')}">(${restaurant.closeImageLink})</c:when>
		</c:choose> 
		</form:label></td>
		<td><input type="file" name="files[5]"/>
		<c:if test="${restaurant.closeImageLink !='' && restaurant.closeImageLink!=null}">
	...
	<tr>
		...
		<c:choose>
			<c:when test="${fn:startsWith(restaurant.businessPortraitImageUrl, 'http://')}">(${restaurant.businessPortraitImageUrl})</c:when>
			<c:when test="${fn:startsWith(restaurant.businessPortraitImageUrl, '/')}">(${restaurant.businessPortraitImageUrl})</c:when>
		</c:choose> 
		</form:label></td>
		<td><input type="file" name="files[0]"/>
		<c:if test="${restaurant.businessPortraitImageUrl !='' && restaurant.businessPortraitImageUrl!=null}">
	...
	<tr>
		...
		<c:choose>
			<c:when test="${fn:startsWith(restaurant.businessLandscapeImageUrl, 'http://')}">(${restaurant.businessLandscapeImageUrl})</c:when>
			<c:when test="${fn:startsWith(restaurant.businessLandscapeImageUrl, '/')}">(${restaurant.businessLandscapeImageUrl})</c:when>
		</c:choose> 
		</form:label></td>
		<td><input type="file" name="files[1]"/>
		<c:if test="${restaurant.businessLandscapeImageUrl !='' && restaurant.businessLandscapeImageUrl!=null}">
	...
	<tr>
		...
		<c:choose>
			<c:when test="${fn:startsWith(restaurant.appCacheIconUrl, 'http://')}">(${restaurant.appCacheIconUrl})</c:when>
			<c:when test="${fn:startsWith(restaurant.appCacheIconUrl, '/')}">(${restaurant.appCacheIconUrl})</c:when>
		</c:choose> 
		</form:label></td>
	...
	<tr>
		...
		Button Icon
		<c:choose>
			<c:when test="${fn:startsWith(restaurant.buttonIconUrl, 'http://')}">(${restaurant.buttonIconUrl})</c:when>
			<c:when test="${fn:startsWith(restaurant.buttonIconUrl, '/')}">(${restaurant.buttonIconUrl})</c:when>
		</c:choose> 
		</form:label></td>
	...
	<tr>
		...
		Marketing Image Url
		<c:choose>
			<c:when test="${fn:startsWith(restaurant.marketingImage, 'http://')}">(${restaurant.marketingImage})</c:when>
			<c:when test="${fn:startsWith(restaurant.marketingImage, '/')}">(${restaurant.marketingImage})</c:when>
		</c:choose> 
		</form:label></td>
		<td><input type="file" name="files[4]"/>
			<c:if test="${restaurant.marketingImage !='' && restaurant.marketingImage!=null}">
	...
	<tr>
		...
		<td><form:input path="alterMarketingText" class="validate[maxSize[100]]"/></td> 
	</tr>
	<tr>
		<td><form:label path="headerImageUrl">
		Header  Image Url
		<c:choose>
			<c:when test="${fn:startsWith(restaurant.headerImageUrl, 'http://')}">(${restaurant.headerImageUrl})</c:when>
			<c:when test="${fn:startsWith(restaurant.headerImageUrl, '/')}">(${restaurant.headerImageUrl})</c:when>
		</c:choose> 
		</form:label></td>
		<td><input type="file" name="files[6]"/>
			<c:if test="${restaurant.headerImageUrl !='' && restaurant.headerImageUrl!=null}">
	...
	<tr>
		<td colspan="2">
			<input type="submit" value="Save Restaurant" style="background-color:#8DC360"/>
			<button type="button" style="background-color:#FEE580" onclick="document.location.href='#index.jsp'">Cancel</button>
		</td>
	</tr>
</table><label id="errorMsg" style="display:none"></label>	
<c:set var="error.restaurantName.exists"><form:errors path="restaurantName"/></c:set>
</form:form>
<hr/>
