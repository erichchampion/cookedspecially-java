...
/***********
 * BEGIN loadHomePage
 * Creates the home page with all sub-menu links
 */
function loadHomePage() {
	console.info("DEBUG >>>> in loadHomePage()");	

	if (globalData != null && typeof(globalData.menus != "undefined") && globalData.menus != null) {
		
		console.info("DEBUG >>>> in creating homepage buttons");
		$.each(globalData.menus, function(i, item) {
			var menuId = item.menuId;
			var menuName = item.name;
			var menuItem = '<a data-role="button" data-theme="c" data-transition="slide" class="topMenuBtn ui-btn ui-shadow ui-btn-corner-all-DISABLED" id="' + menuId + 
				    ...
				    '<span class="ui-btn-text">' + menuName + '</span>' +
				    '</span>' +
			...
			var imageUrl = globalData.menus[ i ].imageUrl;
...
function makeAllPages() {
	...
	if (globalData != null && typeof(globalData.menus != "undefined") && globalData.menus != null) {
		...
		$.each(globalData.menus, function(i, menu) {
			...
			$.each(menu.sections, function(i, section) {
				...
				// iterate through each menu > section > item
				$.each(section.items, function(i, item) {
					...
					//console.info("DEBUG >>>> item name: " + name);
					
					
					//add items to container
					var theItem = '<div class="item isotope-item ' + sectionId + '" id="' + itemId + '"' + 
                      '     onclick="showItemDetail(\'' + menuId + '\',\'' + name + '\',\'' + smallImageUrl + '\',\'' + shortDescription + '\',\'' + price + '\')">' +	
					...
					if (smallImageUrl.length != 0) {
						...
						console.info("DEBUG >>>> itemSelector: " + itemSelector);
						console.info("DEBUG >>>> fullsmallImageUrl: " + fullsmallImageUrl);
		...
		console.info("DEBUG >>>> Loading time: " + (end - start) + " miliseconds"  );
	}
	...
	console.info("DEBUG >>>> END makeAllPages()");	
}
 ...
 */
function addMenuToHomePage_REMOVED(menuId, name, description, imageUrl) {
/*
console.log("DEBUG > BEGIN addMenuToHomePage: " + name);
	...
	if ($("#topMenu " + menuId).length == 0) {
		...
		if (imageUrl.length != 0) {
			...
			console.log("DEBUG > menuSelector: " + menuSelector );	
			console.info("DEBUG >>>> imageUrl: " + imageUrl );
		}
	}
	else {
 ...
 */
 /*
function addMenuToNav_DISABLED(data) {
 ...
 */
function updateOneMenuOnHomePageDISABLED(data) {
/*
 ...
 * Creates all sub-menu links in the home page
 */
...
console.log("DEBUG > BEGIN updateAjaxDataRemote");	
	...
	$('#loadingMsg').html('Downloading data...');
	console.info('DEBUG >>>> Downloading data...');
