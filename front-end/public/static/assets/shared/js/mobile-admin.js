...
function getCheckPreview(id){
    ...
    $('#tableCheckDetails').html('');
  var checkPrivew=  $.ajax({
      url: url,
      dataType: 'json'
    })
    checkPrivew.then(function (data) {
        newCheck = data;
...
function setActiveTable(){
    ...
    console.info(url); 
    var activeTable =$.ajax({
        url: url,
        dataType: 'json'
    })
    activeTable.then(function (data) {
        console.log("AJAX: setActiveTable() SUCCESS"); 
    },function() { 
        console.log("AJAX: setActiveTable() FAILED"); 
    });
    activeTable.complete(function() { 
...
function updateCheck(){
	...
	var obj = jQuery.parseJSON(activeCheck.taxJsonObject);
	var total = activeCheck.roundOffTotal;
	var businessName = "";
	var address1 = "";
	var address2 = "";
	var city = "";
    ...
    for (o=0;o<activeCheck.orders.length;o++){
        ...
        for (i=0;i<activeCheck.orders[o].orderDishes.length;i++){
            ...
            subTotal =subTotal +computedPrice;
        }
    }
    itemsList +='<hr>';
    itemsList +='<div class="left">SUB TOTAL :</div>' ;
    itemsList +='<div class="right">'  + currency + subTotal.toFixed(2)  + '</div>' ;
    itemsList +='<br clear="both"/>';
    if(obj!=null){
    	$.each(obj,function(key, value){
    		
    		itemsList += '<div class="left">' + key + '</div>' ;
    	    itemsList += '<div class="right">'  + currency + value.toFixed(2)  + '</div>' ;
    	    itemsList += '<br clear="both"/>';
    ...
    $('#check_content').html(
        ...
        '<div class="right">' + currency + '<b>' + total  + '</b></div>' + 
		...
		'  </div>' + 
		'</div>' +
		'<div id="invoice Id"><p class="center">Invoie Id : '+activeCheck.invoiceId+'</p>' +
