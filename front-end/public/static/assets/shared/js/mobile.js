...
function addMenuPage(m) {
        ...
        var thePage = '<div data-role="page" id="menu' + menuId + '" data-url="' + menuId + '">' +
                  ...
                  '   <div> <div class="itemDetailPrice"></div>' +
                  '    <div class="increaseSize"></div>' +
                  '    <div class="addToOrder" ></div></div>' + 
...
function showItemDetail(m,s,i) {
    ...
    var id = globalData.menus[m].sections[s].items[i].itemId;
    ...
    $('#' + p + ' .increaseSize').html('<Button value="" class="myButton" onclick="removeQuantity(\'' +id+ '\')">-</Button>&nbsp;<input type="text" min="0" size=3 align="center" style="background-color:white;" disabled id=count'+id+' value="0">&nbsp;<Button value="" class="myButton"  onclick="javascript:addQuantity(\'' +id+ '\')">+</Button>');
    $('#' + p + ' .instructions').html('<label> <b> Special Instructions</b></label><br/><br/><textarea  id='+id+' maxlength="200" style="margin: 0px; width: 437px; height: 30px;" placeholder="Special Instructions"></textarea>');
    $('#' + p + ' .addToOrder').html('<a href="javascript:executeOrder(\'' + m + '\',\'' + s + '\',\'' + i + '\')">Add To Order</a>');
...
console.log("DEBUG > BEGIN updateRestaurantInfo");    
  ...
  var updateResturantInfo = $.ajax(comCookedSpeciallyRestaurantApi, {
        isLocal: false
    })
    updateResturantInfo.then(function (data) {
        if( console && console.log ) {
    ...
    updateResturantInfo.complete(function() { 
