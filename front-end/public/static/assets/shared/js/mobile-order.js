...
function addToOrder(m, s, i) {
    ...
    if ( ! $.isEmptyObject(pendingOrder) ){
        ...
        itemId= item.itemId;
        var instructionsVal  = $('#'+itemId+'').val();
      ...
      /*  $.each(pendingOrder.items, function(i, item) {
       ...
       /* $('#count'+itemId+'').val(pendingOrder.items.length);*/
...
function executeOrder(m, s, i){
	
	for(var j=0;j<quantityCounter;j++){
		addToOrder(m, s, i);
	}
	quantityCounter=0;
}
/*function removeItem(){
	if(count>-1){
	pendingOrder.items.splice(pendingOrder.items ,count);
	}
	else{
		alert("Cart is empty for this item");
	}
	}
*/
function removeFromOrder(id) {
    ...
    $.each(pendingOrder.items, function(i, item) {
        if ( id == item.id && ! _removed ){
            ...
            newOrder.items[newCount] = {
                ...
                instructions: pendingOrder.items[count].instructions,
   ...
   /* $('#count'+id+'').val(pendingOrder.items.length);*/
    updateOrder();    
}
   ...
   var placeOrder = $.ajax({
       ...
       placeOrder.then(function (data) {
            ...
            if ( data.status == "COD"){
                ...
                alert("Order collapsed please order again after clearing from manager screen !") 
            }
