...
function initHomePage() {
    ...
    if (globalData != null && typeof(globalData.menus != "undefined") && globalData.menus != null) {
        ...
        // resize tiles on load and on resize
        $('.topMenuBtn').height( $('.topMenuBtn').width() / 1.2 );
...
function updateMenuPage(m) {
    ...
    if (typeof(globalData.menus[m].menuId) != "undefined" && globalData.menus[m].menuId != null) {
        var menuId = "menu" + globalData.menus[m].menuId;
        var smallImageUrl = "";
        if (typeof(globalData.menus[m].smallImageUrl) != "undefined" && globalData.menus[m].smallImageUrl != null) {
...
function addMenuPage(m) {
    ...
    var thePage = '<div data-role="page" id="menu' + menuId + '" data-url="' + menuId + '">' +
                  ...
                  '  <div class="itemDetailList"></div>' +
                //  '  <div class="menu' + menuId + '-itemDetailList">' +
...
function addSectionToMenuPage(m, s) {
    ...
    if (typeof(globalData.menus[m].sections[s].sectionId) != "undefined" && globalData.menus[m].sections[s].sectionId != null) {
...
function addItemToMenuPage(m,s,i) {
    ...
    if (typeof(item.itemId) != "undefined" && item.itemId != null) {
        ...
        var theItem = '<div class="item isotope-item ' + sectionId + '" id="' + itemId + '"' + 
                      '     onclick="showItemDetailList(\'' + m + '\',\'' + s + '\')">' +     
        ...
        if (typeof(smallImageUrl) != "undefined" && smallImageUrl != null && smallImageUrl.length != 0) {
 ...
 **********/
 ...
 function showItemDetailList(m,s) {
	var p = "menu" +  globalData.menus[m].menuId;
	$('#' + p + ' .shade').show();
    $('#' + p + ' .itemDetailList').show();
	
 }
 
function hideItemDetailList() {
	$('.shade').hide();
    $('.itemDetailList').hide();
...
/***********
 * BEGIN makeDetailList
 * Creates the items list carousel
    Parameters: 
    m - menu index 
    s - section index
 */
function makeDetailList(m,s) {
	console.log('makeDetailList(' + m + ',' + s + ')');
    var p = "menu" +  globalData.menus[m].menuId;
    //var menuId = globalData.menus[m].menuId;
	
	// iterate through section items
	//$.each(globalData.menus[m].sections[s].items, function() {
	for ( var i = 0; i < globalData.menus[m].sections[s].items.length ; i++){
		
		var sectId = globalData.menus[m].sections[s].sectionId;
		var image  = globalData.menus[m].sections[s].items[i].imageUrl;
		
		var itemDetail = '' +
			'<a class="itemDetail">' +
			'	<div class="itemDetailClose" onclick="hideItemDetailList()">Close</div>' +
			'	<div class="itemDetailImg" '+
			'        style="background-image: url(' + comCookedSpeciallyImagePrefix + image + ')">' + globalData.menus[m].sections[s].items[i].name + '</div>' + 
			'	<div class="itemDetailName">' + globalData.menus[m].sections[s].items[i].name + '</div>' + 
			'	<div class="itemDetailDesc">' + globalData.menus[m].sections[s].items[i].description + '</div>' + 
			'	<div class="itemDetailPrice">' + currency + globalData.menus[m].sections[s].items[i].price + '</div>' + 
			'	<div class="addToOrder"><a href="javascript:addToOrder(\'' + m + '\',\'' + s + '\',\'' + i + '\')">Add To Order</a></div>' + 
			'	<!-- div class="itemRating">Popular:</div -->' + 
			'</a>';
		
		//$('.menu' + menuId + '-itemDetailList').append( itemDetail );
		
		// NEED TO ADD SECTIONID
		$('#' + p + ' .itemDetailList').append( itemDetail );
...
//console.log("DEBUG > BEGIN compareAjaxData"); 
    ...
    if (globalDataLocal != null && typeof(globalDataLocal.menus) != "undefined") {
        $.each(globalDataLocal.menus, function() {
            if (typeof(this.modifiedTime) != "undefined" && this.modifiedTime != null) {
...
console.log("DEBUG > BEGIN updateRestaurantInfo");    
    ...
    .done (function (data) {
        ...
        if (restaurantData.appCacheIconUrl != null && restaurantData.appCacheIconUrl.length > 0) {
            $("link#apple-touch-icon").attr('href', restaurantData.appCacheIconUrl);
        }
        if (restaurantData.buttonIconUrl != null && restaurantData.buttonIconUrl.length > 0) {
            $("img#homeBtn").attr('src', restaurantData.buttonIconUrl);
        }
        if (restaurantData.businessLandscapeImageUrl != null && restaurantData.businessLandscapeImageUrl.length > 0) {
        ...
        if (restaurantData.businessName != null && restaurantData.businessName.length > 0) {
...
//      $("div#" + menuId).on( "pagebeforeshow", function(event, ui) {
		...
		// erich: display submenu section only when needed
		if ($("div#" + menuId + " .submenu-items").children().length > 1){
			console.log("Displaying submenu");
			$("div#" + menuId + " .submenu-items").show();
		}
