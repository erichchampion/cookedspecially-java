...
var comCookedSpeciallyRestaurantApi = "";
var localRestaurantApi = "ajax/menus.json";
var appVersion = 1;
var appLatestVersion = 1;
var isLocal = ( location.href.indexOf('http://localhost') > -1 ) ? true : false;
...
function login(){
    ...
    if( !isLocal && typeof(Storage)!=="undefined") {
        if( ! localStorage.getItem("csUserName") || typeof( localStorage.getItem("csUserName") )==="undefined" ) {
            //var uid = prompt('User Id');
            var uid = prompt('User Name');
            
            if ( uid !== null && uid !== "" ){
                localStorage.setItem("csUserName", uid);
             }else{
                login();   
            }
        }
        var csUserName = localStorage.getItem("csUserName");
    ...
    if ( isLocal ){
        ...
        comCookedSpeciallyRestaurantApi = comCookedSpeciallyApiPrefix + "/CookedSpecially/menu/getmenusjsonbyuname?username=" + csUserName;    
    }
...
function addItemToMenuPage(menuId, sectionId, data) {
    if (typeof(data.itemId) != "undefined") {
        ...
        var theItem = '<div class="item ' + sectionId + '" id="' + itemId + '"' +
                      '     onclick="showItem(\'' + menuId + '\',\'' + name + '\',\'' + imageUrl + '\',\'' + shortDescription + '\',\'' + price + '\')">' +
...
function addMenuPage(menuId, name, description) {
    var thePage = '<div data-role="page" id="' + menuId + '" data-url="' + menuId + '">' +
                  ...
                  '    <img onclick="javascript:location.href=\'#home\'" id="homeBtn" style="height: 50px;float:left" src="images/cs-logo.png">' +
                  '    <div id="menu-description"><b>' + name + "</b> | " + description + '</div>' +
                  '    <div id="nav"><a id="navMenuLink" onclick="hideItem();$(\'.navMenu\').toggle()">Menus</a></div>' +
                  '  </div>' +
                  ...
                  '  </div>' +
                  '  <div class="shade" onclick="hideItem()"></div>' +
                  '  <div class="itemDetail">' +        
                  '    <div class="itemDetailClose" onclick="hideItem()">[Close]</div>' +
...
/**********
 * BEGIN checkForUpdates / getAppLatestVersion
 */
...
//function getAppLatestVersion() {
    ...
    $.ajax({
        url: "http://www.bakedspecially.com:8080/static/assets/www/version.php",
...
function showAppVersion() {
    var appInfo = 'About Generic Tablet App\n\n' +
                  'Restaurant: Generic' + '\n' +
                  'Application Version: Beta ' + appVersion + '\n' +
                  //'Latest Version: Beta ' + appLatestVersion;
                  'Latest Version: Beta ' + applicationLatestVersion;
    alert(appInfo);
}
/*
 * END showAppVersion
...
/**********
 * BEGIN showItem and hideItem
 */
function showItem(p, name, image, desc, price) {
    //alert ( 'Name: ' + name + '\nImage: ' + image + '\nDescription: ' + desc+ '\nPrice: ' + price);
    $('.shade').hide();
    $('.itemDetail').hide();
