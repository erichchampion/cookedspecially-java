...
function initHomePage(kitchenScreenId) {
	...
	$.each(allChecksData, function() {
		...
		if(tableName!=null){
			/*if ((tableName.toLowerCase() == "delivery" || tableName.toLowerCase() == "MorningExpress") && currentCheck.deliveryArea != null) {*/
...
function addOrdersToHomePage(tableId, tableName, data,deliveryTime,name,address,phone,paymnetStatus) {
	...
	$.each(data, function() {
		...
		var date =  new Date();
		var date =  new Date();
		var compDate =  new Date(deliveryTime);
		...
		var theLink = '' +
			'<br/>This Check: <a href="javascript:void(0);"  onclick="javascript:printDiv(\'#check' + checkId + '\');">Print</a>'+'<br>'+'<I>'+nameDelivery+'</I> <strong><font size="4">' + orderTimes +'</font></strong><br><I>'+paymentStatusText+'</I><Strong><font size="3">'+paymnetStatus+slashMark+paymentMode+'</font></strong> <br>';
		$(theLink).appendTo(htmlOrderId + " p#links");
