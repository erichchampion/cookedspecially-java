...
function initHomePage(kitchenScreenId) {
	...
	var allOrders = [];
	$("#topMenu").empty();
	if(kitchenScreenId!=undefined){
	globelKitchenId=kitchenScreenId;
	}
...
/*		var additionalChargesValue1 = currentCheck.additionalChargesValue1;
	...
	$.each(allOrders, function() {
		...
		if (currentOrder.status.toLowerCase() != "delivered" && currentOrder.status.toLowerCase() != "delivered") {
			...
			addOrdersToHomePage("#table" + currentOrder.tableId, currentOrder.tableName, myOrderArray,currentOrder.deliveryTime,currentOrder.name,currentOrder.deliveryAddress,currentOrder.phone,currentOrder.total,currentOrder.paymnetStatus);
		}
	});
	// iterate through data file and create orders in homepage
	$.each(allOrders, function() {
		if (this.status.toLowerCase() == "delivered") {
			...
			var myOrderArray = [];
			myOrderArray.push(this);
			addOrdersToHomePage("#table" + tableId, tableName, myOrderArray,deliveryTime,name,deliveryAddress,phone,total,paymnetStatus);
		}
	});
...
var dataValue = [];
function addOrdersToHomePage(tableId, tableName, data,deliveryTime,name,address,phone,total,paymnetStatus) {
		...
		var theOrder = "" + 
			'<ul id="' + tableId + '" data-role="listview" data-inset="true" class="ui-listview ui-listview-inset ui-shadow">' +
				...
				'<p id="links">' +
					...
					'<tr><font size=2> Payment Status :</font><span id="paymentStatus'+orderId+'">'+paymnetStatus+'/'+paymentMode+'</span></tr><br>'+
					'</div>'+
...
function getDeliveryAgent(delBoy){
	var deliveryAgentInfoUrl = _getDeliveryBoy + "restaurantId=" + restaurantId;
	 var getDeliveryAgent = $.ajax({			
	  crossDomain: true,
	  type: 'GET',
	  url: deliveryAgentInfoUrl,
	  dataType: 'json'
     })
	getDeliveryAgent.then(function (data) {
		...
		for (var key in data) {
			 var item = data[key];
			  for (var key2 in item) {
				  ...
				  if(globelKitchenId>0){
					  if(item[key2].fulfillmentCenterId.length>0){
						  $.each(item[key2].fulfillmentCenterId, function( index, value ) {
						//	 alert(globelKitchenId +" : "+value);
				      if(globelKitchenId==value){
				    	  //alert("inserted");
					  	  theOptions = theOptions + '<option value="' + item[key2].name + '">' +item[key2].name+ '</option>';
					  	  optionCount ++;
				  }
			      });
				  }
	...
	getDeliveryAgent.complete(function() { 
...
function setmoneyOutPopUP(invoice,mnyOut){
	...
	if(sel=="CASH" || sel=="COD"){
	$("#moneyOut").show();
//	Akshay - changed this to hide() instead of disabling the screen.
	$("#moneyOutVal1").show();
	$("#moneyOutVal2").show();
	$("#moneyOutVal3").show();
...
function setOrderStatusOD(orderId) {
  ...
  $('option:selected', 'select[name="select-payment-mode"]').removeAttr('selected');
	$('select[name="select-payment-mode"]').find('option[value="PG"]').attr("selected",true);
	document.getElementById("select-payment-mode").value = "PG";
	$("#select-payment-mode").hide();
	$("#select-payment-mode-button > span").text("PG");
	}
	else if(sel=="Unpaid/COD"){
		$("#select-payment-mode").show();
		$("#select-payment-mode-button > span").text("COD");
		$("#select-payment-mode option:contains(COD)").attr('selected', 'selected'); 
	}
	else if(sel=="Paid/SUBSCRIPTION") {
		$("#select-payment-mode").show();
		$("#select-payment-mode-button > span").text("SUBSCRIPTION");
		$("#select-payment-mode option:contains(SUBSCRIPTION)").attr('selected', 'selected'); 
	}
	else{
		$("#select-payment-mode").show();
	...
	$('#setMoneyOut').on("click",function() {
		...
		paymentType =  $("#select-payment-mode"+' option:selected').val();
		}
		if(moneyOut!='' && deliveryBoy!=chooseDeliveryBoy && orderId==setOrderId[0] && paymentType!=''){
		setOrderStatus(setOrderId[0],moneyOut,"Outdelivery",deliveryBoy,paymentType);
		orderId=0;
		}}); 
}
function setOrderStatusDelivered(orderId) {
	...
	$('#setMoneyIn').on("click",function() {
		...
		if(moneyIn !=''&&orderId==setOrderId && setOrderId[0]==orderId){
			setOrderStatus(setOrderId[0],moneyIn,"Confirmdelivery","set");
			orderId=0;
		}});
}
...
//console.log("DEBUG > BEGIN updateAjaxDataRemote");	
	...
	updateAjaxDataRemote.then(function (data) {
	 ...
	 var allCheckApi = 	$.ajax(allChecksApi, {
			isLocal: false
		})
		allCheckApi.then(function (data) {
			if( console && console.log ) {
