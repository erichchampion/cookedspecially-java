...
function submitMobileNumber(){
	...
	cleanPhoneNumber = cleanPhoneNumber.replace(phoneClean, "");
...
function signUp(){
	$.ajax({
		...
		success: function (data) {
			if(data=="SUCCESS") {
				var	custIdParam = "&phone=" + customerData.phone;
				fetchCustomerInfoAndPopulateFields(custIdParam);
				$('#auto-fill-details').show();
				$(".logout").show();
			} else{
...
function verifyOTP(otp){
	$.ajax({
		type: "POST",
		contentType: "application/json; charset=utf-8",
		url: apiPrefix+'/customer/verifyOauthOTP?phone='+customerData.phone+"&OTP="+otp+"&restaurantId=21",
		success: function (data) {
			if (data) {
				$('#sign-in-div').hide();
				$('#auto-fill-details').show();
...
function fetchNewOTP(){
	...
	$.ajax({
		type: "POST",
		contentType: "application/json; charset=utf-8",
		url: apiPrefix+'/customer/fetchNewOTP?phoneNumber='+customerData.phone,
		success: function (data) {
			if(data){
				$("#lblOTPError").text("");
				$("#txtOTP").val("")
